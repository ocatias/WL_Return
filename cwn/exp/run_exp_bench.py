"""
    Just like run_exp.bench except that we move the eval on the training set into the train loop to make it faster and allow fair runtime comparisons
"""
...
from exp.train_utils import eval, Evaluator
...
def train(model, device, loader, optimizer, evaluator, task_type='classification', ignore_unlabeled=False):
    """
    ...
    return curve, evaluator.eval(input_dict)
...
def main(args):
    ...
    if not args.untrained:
        for epoch in range(1, args.epochs + 1):
            ...
            print('Training...')
            epoch_train_curve, train_perf = train(model, device, train_loader, optimizer, evaluator, args.task_type)
            ...
            if runtime_hours > args.max_time:
                break
    ...
    return {
        "mode": mode,
        "loss_train": float(train_loss_curve[best_val_epoch]), 
        "result_train": float(train_curve[best_val_epoch]),
        "result_val": float(valid_curve[best_val_epoch]),
        "result_test": float(test_curve[best_val_epoch]),
