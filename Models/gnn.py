...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_classes, num_tasks, num_layer = 5, emb_dim = 300, 
        ...
        elif self.graph_pooling == "type":
            self.lin_per_dim = torch.nn.ModuleList()
            self.max_type = max_type
            self.pool = global_add_pool
            self.relu = torch.nn.ReLU()
            for _ in range(max_type + 1):
                self.lin_per_dim.append(torch.nn.Linear(self.emb_dim, self.emb_dim))
        else:
            raise ValueError("Invalid graph pooling type.")

        if graph_pooling == "set2set":
            self.graph_pred_linear = torch.nn.Linear(2*self.emb_dim, num_classes*num_tasks)
        else:
            self.graph_pred_linear = torch.nn.Linear(self.emb_dim, num_classes*num_tasks)

    def forward(self, batched_data):
        h_node = self.gnn_node(batched_data)

        if self.graph_pooling == "type":
            dimensional_pooling = []
